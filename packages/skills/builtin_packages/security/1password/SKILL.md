...
- `tmux` available for stable authenticated sessions during Elephant Agent terminal calls (desktop app flow only)
...
### Service Account (recommended for Elephant Agent)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.elephant/.env` (the skill will prompt for this on first load).
...
## Elephant Agent Execution Pattern (desktop app flow)

Elephant Agent terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/elephant-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/elephant-op.sock"
