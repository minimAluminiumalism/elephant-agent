...
| Change | Upstream | Elephant Agent |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `elephant` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Elephant Agent infra |
...
| File operations | Bash commands | Elephant Agent file tools (write_to_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains Elephant Agent-specific adaptations.
