...
**License warning:** OBLITERATUS is AGPL-3.0. NEVER import it as a Python library. Always invoke via CLI (`obliteratus` command) or subprocess. This keeps Elephant Agent's MIT license clean.
...
If not installed, elephant and install from GitHub:
```bash
__GIT_EGG__ https://github.com/elder-plinius/OBLITERATUS.git
cd OBLITERATUS
pip install -e .
