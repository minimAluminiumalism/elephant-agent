...
class MultilingualHybridSearchTest(unittest.TestCase):
    ...
    def test_chinese_exact_and_fuzzy_and_negation(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _index_fact(repo, indexer,
                fact_id="c:weather-crossing",  personal_model_id=pm_id, lens="trait",
                text="我喜欢像站在起雾的路口那样慢慢做决定。", topic="test.weather.crossing")
            _index_fact(repo, indexer,
                fact_id="c:quiet-corner",  personal_model_id=pm_id, lens="rapport",
                text="能量低的时候，我需要一个安静角落。", topic="test.recovery.low_energy")
            _index_fact(repo, indexer,
                fact_id="c:social-positive",  personal_model_id=pm_id, lens="trait",
                text="我喜欢周末约朋友去热闹的 bar。", topic="test.social.positive")
            _index_fact(repo, indexer,
                fact_id="c:social-negative",  personal_model_id=pm_id, lens="trait",
            ...
            # Negation disambiguation: positive-preferring query should prefer positive variant
            self.assertEqual(top_ref("喜欢热闹 聚会 大声说话"), "c:social-positive")
    ...
    def test_english_exact_synonym_metaphor(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _index_fact(repo, indexer,
                fact_id="e:caching-redis", personal_model_id=pm_id, lens="knowledge",
                text="User chose Redis caching over memcached for the aegis project.",
                topic="test.caching.strategy")
            _index_fact(repo, indexer,
                fact_id="e:code-lego", personal_model_id=pm_id, lens="trait",
                text="好的代码像是拼好的乐高，每块都刚好卡在它该在的位置。",
                topic="test.code.metaphor")
            _index_fact(repo, indexer,
                fact_id="e:city-walk", personal_model_id=pm_id, lens="trait",
                text="周末最喜欢做的事是 city walk，在成都的街头漫无目的地走。",
                topic="test.weekend.routine")
            _index_fact(repo, indexer,
                fact_id="e:quiet-corner-en", personal_model_id=pm_id, lens="rapport",
    ...
    def test_cross_lingual_query_variants(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _index_fact(repo, indexer,
                fact_id="x:music-cn", personal_model_id=pm_id, lens="trait",
                text="个人爱好包含音乐和周末听唱片。", topic="test.music.preference")
            _index_fact(repo, indexer,
                fact_id="x:solitude-cn", personal_model_id=pm_id, lens="trait",
                text="孤独有时候是干净的。", topic="test.trait.solitude")
            _index_fact(repo, indexer,
                fact_id="x:city-walk-cn", personal_model_id=pm_id, lens="trait",
    ...
    def test_negation_polarity_disambiguation(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _index_fact(repo, indexer,
                fact_id="np:social-positive-2", personal_model_id=pm_id, lens="trait",
                text="我喜欢周末和朋友们一起去热闹的 bar 喝酒聊天。", topic="test.social.positive_2")
            _index_fact(repo, indexer,
                fact_id="np:social-negative-2", personal_model_id=pm_id, lens="trait",
                text="我不喜欢去人多吵闹的聚会，说话太费劲。", topic="test.social.negative_2")
            _index_fact(repo, indexer,
                fact_id="np:quiet-preference", personal_model_id=pm_id, lens="rapport",
    ...
    def test_low_information_rejection_and_zero_result(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _index_fact(repo, indexer,
                fact_id="z:normal-fact", personal_model_id=pm_id, lens="trait",
            ...
            self.assertEqual(low_info["diagnostics"]["no_match_reason"], "low_information_query")
            ...
            self.assertEqual(zero["match_status"], "no_match")
            self.assertEqual(zero["diagnostics"]["no_match_reason"], "below_min_score")
    ...
    def test_multilingual_synonym_generalization(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _index_fact(repo, indexer,
                fact_id="ml:jp-calm", personal_model_id=pm_id, lens="rapport",
                text="静かな場所で本を読むのが好きです。", topic="test.jp.reading")
            _index_fact(repo, indexer,
                fact_id="ml:kr-citywalk", personal_model_id=pm_id, lens="trait",
                text="주말에 도시를 걷는 것을 좋아합니다.", topic="test.kr.walking")
            _index_fact(repo, indexer,
                fact_id="ml:en-jazz", personal_model_id=pm_id, lens="trait",
                text="I love listening to jazz on weekend mornings.", topic="test.jazz.preference")
            _index_fact(repo, indexer,
                fact_id="ml:cn-fog-2", personal_model_id=pm_id, lens="trait",
            ...
            # KR query with CN variant → should find KR fact
            self.assertEqual(top_ref("도시 걷기", query_variants=("城市漫步",)), "ml:kr-citywalk")
    ...
    def test_topic_exact_and_semantic_merge(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _index_fact(repo, indexer,
                fact_id="tm:optionality", personal_model_id=pm_id, lens="trait",
                text="重要的是保住选择权。", topic="test.choice.optionality")
            _index_fact(repo, indexer,
                fact_id="tm:choice-preference", personal_model_id=pm_id, lens="trait",
                text="做取舍时最不想丢掉的是选择权。", topic="test.choice.preference")
            _index_fact(repo, indexer,
                fact_id="tm:weekend-routine", personal_model_id=pm_id, lens="trait",
    ...
    def test_empty_query_and_no_searcher_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # No searcher (None) → unified_recall falls back to lexical ranker
            hits = unified_recall(
                UnifiedRecallRequest(
                    query="concise",
    ...
    def test_large_batch_ranking_stability(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(15):
                _index_fact(repo, indexer,
                    fact_id=f"dist:{i:03d}", personal_model_id=pm_id, lens="knowledge",
            ...
            for i in range(5):
                _index_fact(repo, indexer,
                    fact_id=f"arch:{i:03d}", personal_model_id=pm_id, lens="knowledge",
