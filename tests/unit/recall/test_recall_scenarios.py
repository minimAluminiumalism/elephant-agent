...
SCENARIOS_PATH = ROOT / "tests" / "scenarios" / "recall" / "scenarios.yaml"
SCENARIO_DIR = ROOT / "tests" / "scenarios" / "recall"


class RecallScenarioFixtureTests(unittest.TestCase):
    def test_recall_scenario_manifest_is_stable(self) -> None:
    ...
    def test_recall_scenario_files_exist(self) -> None:
        manifest = json.loads(SCENARIOS_PATH.read_text(encoding="utf-8"))

        for scenario in manifest["scenarios"]:
            self.assertTrue((SCENARIO_DIR / scenario["file"]).exists(), scenario["file"])

    def test_recall_scenario_assertions_are_explicit(self) -> None:
        manifest = json.loads(SCENARIOS_PATH.read_text(encoding="utf-8"))
